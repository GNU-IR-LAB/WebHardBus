...
class FileStar() {
    ...
    fun createWorkbook(data: List<String>, flag: Boolean) {
        ...
        if(flag) {
            // 새 시트 생성
            val sheet = workbook.createSheet("file_star manga")
            ...
            try {
                val outputStream = FileOutputStream("File_Star_titles.xlsx")
            ...
            // 새 시트 생성
            val sheet = workbook.createSheet("file_star novel")
            ...
            try {
                val outputStream = FileOutputStream("File_Star_Novel_titles.xlsx")
                workbook.write(outputStream)
                workbook.close()
                outputStream.close()
            } catch (e: Exception) {
    ...
    fun crawling(): MutableList<String> {
        ...
        try {
            var page = 1
            driver.get("https://filestar.co.kr/category/web/14100/")
            val jse = driver as JavascriptExecutor
            sleep(4000)

            while (true) {
                page += 1
                val table = driver.findElement(By.id("category-contents-list-data-text"))
                val titlesElements: List<WebElement> = table.findElements(By.className("tit"))
                ...
                try {
                    val next = driver.findElement(By.className("next"))
                    next.click()
                } catch (e: Exception) {
                    println("종료\n" +
                            "페이지 수 : $page")
    ...
    fun crawlingNovel(): MutableList<String> {
        ...
        try {
            var page = 1
            driver.get("https://filestar.co.kr/category/web/14000/")
            val jse = driver as JavascriptExecutor
            sleep(4000)
...
fun main() {
    val fileStar = FileStar()
    val data = fileStar.crawlingNovel()
    fileStar.createWorkbook(data,false)
}