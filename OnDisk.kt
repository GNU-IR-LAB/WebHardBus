...
class OnDisk() {
    ...
    fun createWorkbook(data: List<String>, flag: Boolean) {
        ...
        if(flag) {
            // 새 시트 생성
            val sheet = workbook.createSheet("on_disk manga")
            ...
            try {
                val outputStream = FileOutputStream("On_Disk_titles.xlsx")
            ...
            // 새 시트 생성
            val sheet = workbook.createSheet("on_disk novel")
            ...
            try {
                val outputStream = FileOutputStream("On_Disk_Novel_titles.xlsx")
                workbook.write(outputStream)
                workbook.close()
                outputStream.close()
            } catch (e: Exception) {
    ...
    fun crawling(): MutableList<String> {
        ...
        try {
            driver.get("https://ondisk.co.kr/")
            driver.switchTo().frame("mainFrame")

            sleep(10000)

            val categoryButton = driver.findElement(By.xpath("//*[@id=\"gnb\"]/ul/li[6]/a/img"))
            categoryButton.click()

            sleep(5000)

            for (i in 2..500) {
                val table = driver.findElement(By.cssSelector("#contents_list"))
                var titlesElements: List<WebElement> = table.findElements(By.className("summy_link"))

                // 각 요소의 텍스트를 추출하여 리스트에 추가
                for (titleElement in titlesElements) {
                    titlesList.add(titleElement.getAttribute("title"))
                    println(titleElement.getAttribute("title"))
                }
                val nextButton = driver.findElement(By.xpath("//*[@id=\"paging\"]/p/a[17]/img"))
                nextButton.click()
                sleep(5000)
            }
...
fun main() {
    val onDisk = OnDisk()
    val data = onDisk.crawlingNovel()
    onDisk.createWorkbook(data, false)
}