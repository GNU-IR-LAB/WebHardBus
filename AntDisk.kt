...
class AntDisk() {
    ...
    fun createWorkbook(data: List<String>) {
        ...
        // 새 시트 생성
        val sheet = workbook.createSheet("ant_disk manga")
        ...
        try {
            val outputStream = FileOutputStream("Ant_Disk_titles.xlsx")
    ...
    fun crawling(): MutableList<String> {
        ...
        try {
            driver.get("https://antdisk.kr/storage.php?section=CTN&nLimit=100")
            val jse = driver as JavascriptExecutor
            sleep(4000)

            for (i in 1..638) {

                val table = driver.findElement(By.id("contentList_Table"))
                val titlesElements: List<WebElement> = table.findElements(By.className("okclick"))
                ...
                jse.executeScript("directStoragePage($i,'direct');")
...
fun main() {
    val antDisk = AntDisk()
    val data = antDisk.crawling()
    antDisk.createWorkbook(data)
}